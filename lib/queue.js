...
class Queue {
    constructor(connection, name = 'default', options) {
        ...
        // 创建任务表
        let query = `CREATE TABLE IF NOT EXISTS ${this.table} (id INTEGER PRIMARY KEY ASC AUTOINCREMENT,` +
        ...
        this.db.run(query, function (err) {
            if (err) {
                console.error('创建表错误', query, err);
                throw err;
            }
            console.log('创建表成功', query);
        });
    ...
    dequeue(options, callback) {
        ...
        console.log('消费时执行的sql查询语句', querySql);
